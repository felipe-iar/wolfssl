...
#include "wolfssl_thread.h"
                ...
                void wolfssl_thread_create(void);
                static void wolfssl_thread_func(void * pvParameters);
                void rtos_startup_err_callback(void * p_instance, void * p_data);
                void rtos_startup_common_init(void);
extern uint32_t g_fsp_common_thread_count;

                const rm_freertos_port_parameters_t wolfssl_thread_parameters =
                {
                    .p_context = (void *) NULL,
                };
                ...
                void wolfssl_thread_create (void)
                {
                    ...
                    #if 0
                    wolfssl_thread = xTaskCreateStatic(
                    #else
                    BaseType_t wolfssl_thread_create_err = xTaskCreate(
                    #endif
                        wolfssl_thread_func,
                        ...
                        1,
                        #if 0
                        (StackType_t *)&wolfssl_thread_stack,
                        (StaticTask_t *)&wolfssl_thread_memory
                        #else
                        & wolfssl_thread
                        #endif
                    );
                    ...
                    #if 0
                    if (NULL == wolfssl_thread)
                    {
                        rtos_startup_err_callback(wolfssl_thread, 0);
                    }
                    #else
                    if (pdPASS != wolfssl_thread_create_err)
                    {
                        rtos_startup_err_callback(wolfssl_thread, 0);
                    }
                    #endif
                ...
                static void wolfssl_thread_func (void * pvParameters)
                {
                    ...
                    wolfssl_thread_entry(pvParameters);
                }
